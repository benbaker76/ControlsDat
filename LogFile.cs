...
namespace ControlsDat
{
    class LogFile
    {
        ...
        /* public static void VerboseWriteEntry(string Method, string Class, string Output)
        {
            if (Settings.General.VerboseLogging)
                WriteEntry(String.Format("{0} ({1}) -> {2}", Method, Class, Output));
        } */
        ...
        public static void OutputSystemConfiguration()
        {
            try
            {
                WriteEntry("CPWizard " + Global.Version);
